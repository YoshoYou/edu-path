import { LucideMenu, LucideMenuSquare, LucideSquareMenu, MenuIcon, SquareMenuIcon, X } from "lucide-react"
    ...
    <div className="sm:hidden block  ">
      ...
      menuOpen==='open'?  <X className="w-8 h-8 text-emerald-600 cursor-pointer hover:text-emerald-400" onClick={()=>{toggleMenu('close')}}/>:<LucideMenuSquare className="w-8 h-8 text-emerald-600 cursor-pointer hover:text-emerald-400 " onClick={()=>{toggleMenu('open')}} />
        ...
        <ul className={`fixed top-19 bg-gray-50 -right-150 ${menuAnimation}    text-2xl font-bold flex flex-col gap-10 w-full pt-5 px-5 border-b border-gray-300 shadow-gray-300  pb-8 shadow-2xs`}>
                            {
                                List.map((item,index)=>(
                            <li className={`cursor-pointer text-gray-500  hover:bg-linear-to-br hover:from-emerald-600 hover:to-teal-600 hover:bg-clip-text hover:text-transparent`}
                            key={index}>
                                {item}
                                </li>
                                ))
                            ...
                            <div className="text-white text-center   bg-linear-to-br from-emerald-600 to-teal-600 p-2 rounded-lg hover:from-emerald-400 cursor-pointer">
                                    <button >LogIn</button>
                            </div>
        </ul>
    </div>
